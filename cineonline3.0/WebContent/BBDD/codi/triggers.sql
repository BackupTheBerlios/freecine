...
--funcio per a comprobar que quan eliminem una sala no te cap sessio associada amb data igual o posterior a avui
...
CREATE OR REPLACE FUNCTION insert_sess() RETURNS TRIGGER AS'
	...
	BEGIN
		...
		IF NOT FOUND THEN
			RAISE EXCEPTION ''SE01-Sessió: clau foranea id_pelicula no existent a pelicula'';
		END IF;
		...
		WHERE NEW.id_sala=sala.id;
		IF NOT FOUND THEN
			RAISE EXCEPTION ''SE02-Sessió: clau foranea id_sala no existent a sala'';
		END IF;
...
CREATE OR REPLACE FUNCTION insert_comp_res() RETURNS TRIGGER AS'
	...
	BEGIN
		...
		IF FOUND THEN
			RAISE EXCEPTION ''CR10- Compra_reserva: Ja hi ha una compra_reserva en la mateixa sessio butaca'';
		END IF;
		SELECT *  INTO descarta 
		FROM compra_reserva
...
CREATE OR REPLACE FUNCTION elim_pel() RETURNS TRIGGER AS'
	...
	BEGIN
		...
		AND sessio.data_hora_inici >= NOW();
		IF FOUND THEN
			RAISE EXCEPTION ''PE08- Pelicula: No es pot eliminar la pel좵la perqu柨i han sessions futures associades, elimina primer les sessions'';
		ELSE
			DELETE FROM sessio
			WHERE sessio.id_pelicula = OLD.id;
		END IF;
