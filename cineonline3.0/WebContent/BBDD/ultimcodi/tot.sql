...
CREATE OR REPLACE FUNCTION insert_comp_res() RETURNS TRIGGER AS'
	...
	BEGIN
		...
		IF NEW.data_hora+interval ''5 days'' < data_hora_sessio THEN
			RAISE EXCEPTION ''CR09- Compra_reserva: No es pot comprar ni reservar fins que faltin 5 dies per a la sessió'';
		END IF;
		SELECT *  INTO descarta 
		FROM compra_reserva
		...
		IF FOUND THEN
			RAISE EXCEPTION ''CR10- Compra_reserva: Ja hi ha una compra_reserva en la mateixa sessi� i butaca'';
		END IF;
		SELECT *  INTO descarta 
		FROM compra_reserva
...
CREATE OR REPLACE FUNCTION elim_pel() RETURNS TRIGGER AS'
	...
	BEGIN
		...
		AND sessio.data_hora_inici >= NOW();
		IF FOUND THEN
			RAISE EXCEPTION ''PE08- Pelicula: No es pot eliminar la pel�cula perqu� hi han sessions futures associades, elimina primer les sessions'';
		ELSE
			DELETE FROM sessio
			WHERE sessio.id_pelicula = OLD.id;
		END IF;
...
--funci�per a modificar un gènere, no es pot modificar res
...
INSERT INTO genere (descripcio) VALUES ('Comedia');
INSERT INTO genere (descripcio) VALUES ('Drama');
INSERT INTO genere (descripcio) VALUES ('Tragicomedia');
INSERT INTO genere (descripcio) VALUES ('Terror');
INSERT INTO genere (descripcio) VALUES ('Intriga');
INSERT INTO genere (descripcio) VALUES ('Musical');
INSERT INTO genere (descripcio) VALUES ('Ciencia-ficcio');
