 ...
 public class GestioPeliculesServlet extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {
	...
	private void afegirPeliculaAction() throws ServletException, IOException{
		...
		try {
			...
			String any = request.getParameter("anny");
			int anny = -1;
			String durad = request.getParameter("durada");
			int durada =-1;
			String nacionalitat = request.getParameter("nacionalitat");
			String edatRecomenad = request.getParameter("edatRecomenada");
			int edatRecomenada =-1;
			
			
			System.err.println("titol["+titol+"]");
			if(titol==null || titol.equals("") ){
				System.err.println("titol["+titol+"]");
				throw new ControladorException("El camp titol no pot ser buit");
			}
				
				
		    if(any==null || any.equals("")){
		    	throw new ControladorException("El camp any no pot ser buit");
			}else{
				anny=Integer.parseInt(any);
			}
			

			if(durad==null || durad.equals("")){
				throw new ControladorException("El camp durada no pot ser buit");
			}else{
				durada=Integer.parseInt(durad);
			}
			
			

			if(edatRecomenad==null || edatRecomenad.equals("")){
				throw new ControladorException("El camp edatRecomenada no pot ser buit");
			}else{
				edatRecomenada=Integer.parseInt(edatRecomenad);
			}
