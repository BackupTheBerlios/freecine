 ...
 public class GestioClientServlet extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {
	...
	private void detallPeliculaAction() throws ServletException, IOException {
		...
		try {
			...
			int idNac =Integer.parseInt(pelicula.getNacionalitat());
			
			pelicula.setNacionalitat(""+(ctrl.getNacionalitat(idNac)).elementAt(1));
			
		    rd = getServletContext().getRequestDispatcher(urlExit);
		    rd.forward(request, response);
		    
		} catch (ControladorException e) {
		    RequestDispatcher rd = getServletContext().getRequestDispatcher(urlError+"?error= No es pot obtenir la pelicula");
		    rd.forward(request, response);
		}
	}
	...
	private void llistarAction() throws ServletException, IOException {		
		...
		try {
			...
			RequestDispatcher rd = getServletContext().getRequestDispatcher(urlError+"?error=No es poden obtenir les pelicules");
		    rd.forward(request, response);
		}
	...
	private void llistarSessionsCartelleraAction() throws ServletException, IOException {
		...
		try {
		    ...
		    RequestDispatcher rd = getServletContext().getRequestDispatcher(urlError+"?error=No es pot obtenir les sessions per la cartellera");
		    rd.forward(request, response);
		}
	...
	private void vendaAction() throws ServletException, IOException {
		...
		if(idSessio != null){
			try {
			    ...
			    RequestDispatcher rd = getServletContext().getRequestDispatcher(urlError+"?error=No es poden obtenir les sessions");
			    rd.forward(request, response);
			}
		}else{
			RequestDispatcher rd = getServletContext().getRequestDispatcher(urlError+"?error=No pot accedir al idSessio nulla");
		    rd.forward(request, response);
		}
	...
	private void cercaAction() throws ServletException, IOException {
		...
		try {
		    ...
		    RequestDispatcher rd = getServletContext().getRequestDispatcher(urlError+"?error= No es pot cercar la sessio per la data actual");
		    rd.forward(request, response);
		}
	...
	private void continuarVendaAction() throws ServletException, IOException{
		...
		try{
			...
			rd = getServletContext().getRequestDispatcher(urlError+"?error=Les butaques no es poden seleccionar, torni a començar el procés desde l'inici");
			rd.forward(request,response);
		}
	}
	...
	private void confirmarAction() throws ServletException, IOException {
		...
		try {
			...
			rd = getServletContext().getRequestDispatcher(urlError+"?error=No es pot realitzar la compra/reserva pot ser que les butaques ja hagis sigut comprades per un altre client");
			rd.forward(request,response);
		}
	...
	private void cercaPeliculesAction() throws ServletException, IOException{
		...
		try {
		    ...
		    RequestDispatcher rd = getServletContext().getRequestDispatcher(urlError+"?error=No es pot realitzar la cerca de pelicules");
		    rd.forward(request, response);
		}
	}
	...
	private void cercaSessioAction() throws ServletException, IOException {
		...
		try {
		    ...
		    RequestDispatcher rd = getServletContext().getRequestDispatcher(urlError+"?error=No es pot realitzar la cerca de sessions");
		    rd.forward(request, response);
		}
