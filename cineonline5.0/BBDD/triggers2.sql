...
CREATE OR REPLACE FUNCTION modif_sess() RETURNS TRIGGER AS'
	...
	BEGIN	
		...
		IF NEW.data_hora_fi != OLD.data_hora_fi THEN
			RAISE E?CEPTION ''SE10- Sessio: El camp data_hora_fi no es pot modificar'';
		END IF;
...
CREATE TRIGGER eliminar_compra_reserva BEFORE DELETE O? compra_reserva  
...
CREA?E TRIGGER modificar_pelicula BEFORE UPDATE ON pelicula  
